...
int main() {
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        auto inicio = chrono::high_resolution_clock::now();
        mergesortExterno(nombreEntrada, nombreSalida, M, B, mejorA);
        auto fin = chrono::high_resolution_clock::now();
    ...
    for (int mult = 4; mult <= 60; mult += 4) {
        size_t N = mult * 1024 * 1024; // cantidad de elementos
